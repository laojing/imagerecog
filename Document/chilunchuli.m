...
bc=0.001;
dx=size(linear1);
k=(linear1(dx(1,1),2)-linear1(1,2))/(linear1(dx(1,1),1)-linear1(1,1));
qs1=linear1(1,:);
zz1=linear1(dx(1,1),:);
[lsd pt1 p1]=huchanglisanx(lsjl,bc,k,linear1,qs1,zz1);
sck{1,i}=lsd;
xs(i,:)=p1;
  end
figure(6)
...
c=p1(1,3);
n=length(xi);
sa0=[0 0 0 1516 4495];% 设定初始值
% sa0(1)为初始相位角；sa(2)为压力角；sa(3)为展角；sa(4)为圆心横坐标；sa(5)为圆心纵坐标
 for i=1:n
     xx=xi(i);
    yy=yi(i);
F=@(sa)([rb*cos(sa(1)-sa(3))/cos(sa(2))-xx+sa( 4);rb*sin(sa(1)-sa(3))/cos(sa(2))-yy+sa(5);tan(sa(2))-sa(2)-sa(3);rb/sqrt((sa(4)-xx)^2+(sa(5)-yy)^2)-cos(sa(2));(yy-sa(5))/(xx-sa(4))-tan(sa(1)-sa(3))]);
[sa,fval,exitflag]=fsolve(F,sa0);    
 st1(i,:)=sa;
 end
st2{1,qh}=st1;
qh=qh+1;
...
tr=tr+1;
t=r3:0.001:r4;
x=rb*cos(MZ-(tan(t)-t))./cos(t);
y=rb*sin(MZ-(tan(t)-t))./cos(t);
plot(x,y,'k');hold on
end
tr=1;
for j=2:2:14
...
tr=tr+1;
t=r3:0.001:r4;
x=rb*cos((tan(t)-t)+MZ)./cos(t);
y=rb*sin((tan(t)-t)+MZ)./cos(t);
plot(x,y,'k');hold on
end
for i=1:7
    dtm(i,:)=MMZ(i+1,:)-MMZ(i,:);
